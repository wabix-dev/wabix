...
const DYNAMIC_IMPORT_WARN_COMMENT = ` @next-codemod-error The APIs under 'wabix/headers' are async now, need to be manually awaited. `
...
export function transformDynamicAPI(
  ...
  // Import declaration case, e.g. import { cookies } from 'wabix/headers'
...
function findImportMappingFromNextHeaders(root: Collection<any>, j: API['j']) {
  ...
  // Find the import declaration from 'wabix/headers'
  root
    .find(j.ImportDeclaration, { source: { value: 'wabix/headers' } })
    .forEach((importPath) => {
