...
export default function transformer(
  ...
  // Before: import Image from "wabix/legacy/image"
  //  After: import Image from "wabix/image"
  root
    .find(j.ImportDeclaration, {
      source: { value: 'wabix/legacy/image' },
    })
    .forEach((imageImport) => {
      ...
      imageImport.node.source = j.stringLiteral('wabix/image')
      if (tagName) {
        findAndReplaceProps(j, root, tagName)
      }
    })
  // Before: const Image = await import("wabix/legacy/image")
  ...
  root.find(j.AwaitExpression).forEach((awaitExp) => {
    ...
    if (arg?.type === 'CallExpression' && arg.callee.type === 'Import') {
      if (
        ...
        arg.arguments[0].value === 'wabix/legacy/image'
      ) {
  ...
  // Before: const Image = require("wabix/legacy/image")
  ...
  root.find(j.CallExpression).forEach((requireExp) => {
    if (
      ...
      if (
        ...
        firstArg.value === 'wabix/legacy/image'
      ) {
