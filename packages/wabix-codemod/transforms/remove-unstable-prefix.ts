...
export default function transformer(
  ...
  try {
    ...
    // Handle ES6 imports: import { unstable_cacheTag } from 'wabix/cache'
    root
      .find(j.ImportDeclaration, { source: { value: 'wabix/cache' } })
      .forEach((path) => {
        path.node.specifiers?.forEach((specifier) => {
          if (
            ...
            // Handle namespace imports: import * as cache from 'wabix/cache'
            cacheVariables.add(specifier.local.name)
          }
        })
      })

    // Handle export statements: export { unstable_cacheTag } from 'wabix/cache'
    root
      .find(j.ExportNamedDeclaration, { source: { value: 'wabix/cache' } })
      .forEach((path) => {
        path.node.specifiers?.forEach((specifier) => {
          if (
    ...
    // Handle require('wabix/cache') calls and destructuring
    root
      .find(j.CallExpression, { callee: { name: 'require' } })
      .forEach((path) => {
        if (
          path.node.arguments[0]?.type === 'StringLiteral' &&
          path.node.arguments[0].value === 'wabix/cache'
        ) {
          // Track variable assignments: const cache = require('wabix/cache')
          ...
          // Handle destructuring: const { unstable_cacheTag } = require('wabix/cache')
          if (
            parent?.type === 'VariableDeclarator' &&
    ...
    // Handle await import('wabix/cache') calls and destructuring
    root.find(j.AwaitExpression).forEach((path) => {
      ...
      if (
        ...
        arg.arguments[0].value === 'wabix/cache'
      ) {
        // Track variable assignments: const cache = await import('wabix/cache')
        const parent = path.parent?.node
        if (
          parent?.type === 'VariableDeclarator' &&
          parent.id?.type === 'Identifier'
        ...
        // Handle destructuring: const { unstable_cacheTag } = await import('wabix/cache')
        if (
          parent?.type === 'VariableDeclarator' &&
          parent.id?.type === 'ObjectPattern'
        ) {
    ...
    // Handle .then() chains: import('wabix/cache').then(({ unstable_cacheTag }) => ...)
    root.find(j.CallExpression).forEach((path) => {
      if (
        ...
        path.node.callee.object.arguments[0].value === 'wabix/cache' &&
    ...
    root.find(j.MemberExpression).forEach((path) => {
      ...
      // Handle direct property access: require('wabix/cache').unstable_cacheTag
      if (
        node.object?.type === 'CallExpression' &&
        node.object.callee?.type === 'Identifier' &&
        node.object.callee.name === 'require' &&
        node.object.arguments[0]?.type === 'StringLiteral' &&
        node.object.arguments[0].value === 'wabix/cache'
      ) {
        if (
          node.computed &&
