...
const program = new Command(packageJson.name)
  .version(
    packageJson.version,
    '-v, --version',
    'Output the current version of create-wabix-app.'
  )
  .argument('[directory]')
  ...
  .option(
    ...
    'Reset the preferences saved for create-wabix-app.'
  )
  .option(
    '--skip-install',
  ...
  from the official Wabix repo or a public GitHub URL. The URL can use
...
async function run(): Promise<void> {
  const conf = new Conf({ projectName: 'create-wabix-app' })
      ...
      const { setupChoice } = await prompts(
        {
          type: 'select',
          name: 'setupChoice',
          message: 'Would you like to use the recommended Wabix defaults?',
          choices,
          initial: 0,
        },
        {
...
async function notifyUpdate(): Promise<void> {
  try {
    if ((await update)?.latest) {
      ...
      const updateMessage = `${global[packageManager]} create-wabix-app${pkgTag}`
      console.log(
        yellow(bold('A new version of `create-wabix-app` is available!')) +
          '\n' +
