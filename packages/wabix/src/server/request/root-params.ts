...
export function getRootParam(paramName: string): Promise<ParamValue> {
  const apiName = `\`import('wabix/root-params').${paramName}()\``
  ...
  if (actionStore) {
    ...
    if (actionStore.isAction && workUnitStore.phase === 'action') {
      ...
      throw new Error(
        `${apiName} was used inside a Server Action. This is not supported. Functions from 'wabix/root-params' can only be called in the context of a route.`
      )
    }
  }
