...
import findUp from 'wabix/dist/compiled/find-up'
...
import { loadEnvConfig, updateInitialEnv } from '@wabix/env'
...
import type { ZodError } from 'wabix/dist/compiled/zod'
...
function assignDefaultsAndValidate(
  ...
  if (
    ...
    const bytes =
      require('wabix/dist/compiled/bytes') as typeof import('wabix/dist/compiled/bytes')
...
function getCacheKey(
  ...
  // The wabix.config.js is unique per project, so we can use the dir as the major key
...
export default async function loadConfig(
  ...
  let configFileName = 'wabix.config.js'
  ...
  if (path?.length) {
    ...
    try {
      ...
      } else if (configFileName === 'wabix.config.ts') {
      ...
      // TODO: Modify docs to add cases of failing wabix.config.ts transformation
      curLog.error(
    ...
    const unsupportedConfig = findUp.sync(
      [
        ...
        ...((process.features as any).typescript ? [] : ['wabix.config.mts']),
        `${configBaseName}.json`,
        `${configBaseName}.jsx`,
        `${configBaseName}.tsx`,
      ],
    ...
    if (unsupportedConfig?.length) {
      throw new Error(
        `Configuring Next.js via '${basename(
          unsupportedConfig
        )}' is not supported. Please replace the file with 'wabix.config.js', 'wabix.config.mjs', or 'wabix.config.ts'.`
      )
    }
  }
  ...
  // reactRoot can be updated correctly even with no wabix.config.js
  const completeConfig = assignDefaultsAndValidate(
    dir,
