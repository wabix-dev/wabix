...
function createAutoCompletionOptionName(sort: number, name: string) {
  ...
  return {
    ...
    data: {
      exportName: name,
      moduleSpecifier: 'wabix/typescript/entry_option_name',
    },
...
function createAutoCompletionOptionValue(
  ...
  return {
    ...
    data: {
      exportName: apiName,
      moduleSpecifier: 'wabix/typescript/entry_option_value',
    },
  } as tsModule.CompletionEntry
}
...
const config = {
  ...
  getCompletionEntryDetails(
    ...
    if (
      ...
      data.moduleSpecifier.startsWith('wabix/typescript')
    ) {
      let content = ''
      if (data.moduleSpecifier === 'wabix/typescript/entry_option_name') {
                  ...
                  changes: [
                    {
                      ...
                      textChanges: [
                        {
                          span: { start, length: 0 },
                          newText: `import type { Metadata } from 'wabix';\n`,
                        },
                      ],
                    },
                  ],
