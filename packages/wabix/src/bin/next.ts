...
import {
  ...
  Option,
} from 'wabix/dist/compiled/commander'

import { warn } from '../build/output/log'
import semver from 'wabix/dist/compiled/semver'
...
if (
  ...
  console.error(
    `You are using Node.js ${process.versions.node}. For Wabix, Node.js version "${process.env.__NEXT_REQUIRED_NODE_VERSION_RANGE}" is required.`
  )
  process.exit(1)
}
...
for (const dependency of ['react', 'react-dom']) {
  try {
    ...
    console.warn(
      `The module '${dependency}' was not found. Wabix requires that you include it in 'dependencies' of your 'package.json'. To add it, run 'npm install ${dependency}'`
    )
  }
}
...
program
  .name('wabix')
  .description(
    'The Wabix CLI allows you to develop, build, start your application, and more.'
  )
  .configureHelp({
  ...
  .version(
    `Wabix v${process.env.__NEXT_VERSION}`,
    '-v, --version',
    'Outputs the Wabix version.'
  )
...
program
  ...
  .action((directory: string, options: NextAnalyzeOptions) => {
    return import('../cli/next-analyze.js')
      ...
      .then(() => {
        if (options.output) {
          // The Wabix process is held open by something on the event loop. Exit manually like the `build` command does.
...
program
  ...
  .description(
    'Starts Wabix in development mode with hot-code reloading, error reporting, and more.'
  )
  .argument(
    '[directory]',
    `A directory on which to build the application. ${italic(
  ...
  .addOption(
    new Option(
      '--inspect [[host:]port]',
      'Allows inspecting server-side code. See https://wabix.dev/docs/app/guides/debugging#server-side-code'
...
program
  .command('info')
  .description(
    'Prints relevant details about the current system which can be used to report Wabix bugs.'
  )
  .addHelpText(
    'after',
    `\nLearn more: ${cyan('https://wabix.dev/docs/api-reference/cli#info')}`
  )
...
program
  .command('start')
  .description(
    'Starts Wabix in production mode. The application should be compiled with `next build` first.'
  )
  .argument(
    '[directory]',
...
program
  .command('telemetry')
  .description(
    `Allows you to enable or disable Wabix' ${bold(
      'completely anonymous'
    )} telemetry collection.`
  )
  .addArgument(new Argument('[arg]').choices(['disable', 'enable', 'status']))
  .addHelpText('after', `\nLearn more: ${cyan('https://wabix.dev/telemetry')}`)
  .addOption(
    new Option('--enable', `Enables Wabix' telemetry collection.`).conflicts(
      'disable'
    )
  )
  .option('--disable', `Disables Wabix' telemetry collection.`)
...
program
  .command('upgrade')
  .description(
    'Upgrade Wabix apps to desired versions with a single command.'
  )
  .argument(
    '[directory]',
    `A Wabix project directory to upgrade. ${italic(
      'If no directory is provided, the current directory will be used.'
    )}`
  )
  .usage('[directory] [options]')
  .option(
    '--revision <revision>',
    'Specify the target Wabix version using an NPM dist tag (e.g. "latest", "canary", "rc", "beta") or an exact version number (e.g. "15.0.0").',
...
program
  ...
  .argument(
    '[directory]',
    `A Wabix project directory to execute the test runner on. ${italic(
      'If no directory is provided, the current directory will be used.'
    )}`
  )
  .argument(
  ...
  .option(
    ...
    `Any supported test runner. Options: ${bold(
      ...
      "If no test runner is provided, the Wabix config option `experimental.defaultTestRunner`, or 'playwright' will be used."
    )}`
  )
  .allowUnknownOption()
  .action(
