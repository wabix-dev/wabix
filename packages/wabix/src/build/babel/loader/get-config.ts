...
import { createConfigItem, loadOptions } from 'wabix/dist/compiled/babel/core'
import loadFullConfig from 'wabix/dist/compiled/babel/core-lib-config'
...
function getPlugins(
  ...
  const reactRefreshItem = hasReactRefresh
    ? createConfigItem(
        [
          require('wabix/dist/compiled/react-refresh/babel') as typeof import('wabix/dist/compiled/react-refresh/babel'),
          { skipEnvCheck: true },
        ],
        { type: 'plugin' }
      )
  ...
  const transformDefineItem = createConfigItem(
    [
      require.resolve('wabix/dist/compiled/babel/plugin-transform-define'),
      {
  ...
  const commonJsItem = isNextDist
    ? createConfigItem(
        require('wabix/dist/compiled/babel/plugin-transform-modules-commonjs') as typeof import('wabix/dist/compiled/babel/plugin-transform-modules-commonjs'),
        { type: 'plugin' }
      )
    : null
...
function checkCustomBabelConfigDeprecation(
  ...
  const isPresetReadyToDeprecate =
    !presets ||
    presets.length === 0 ||
    (presets.length === 1 && presets[0] === 'wabix/babel')
  const pluginReasons = []
