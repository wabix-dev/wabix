...
import ReactRefreshWebpackPlugin from 'wabix/dist/compiled/@next/react-refresh-utils/dist/ReactRefreshWebpackPlugin'
import { yellow, bold } from '../lib/picocolors'
import crypto from 'crypto'
import { webpack } from 'wabix/dist/compiled/webpack/webpack'
...
import JSON5 from 'wabix/dist/compiled/json5'
...
const reactRefreshLoaderName =
  'wabix/dist/compiled/@next/react-refresh-utils/dist/loader'
...
export default async function getBaseWebpackConfig(
  ...
  const middlewareLayerLoaders = [
    ...
    // for middleware to tree shake the unused default optimized imports like "wabix/server".
    // This will cause some performance overhead but
    // acceptable as Babel will not be recommended.
    getSwcLoader({
      serverComponents: true,
                      ...
                      config.experimental.fallbackNodePolyfills === false
                        ...
                        : {
                            assert: require.resolve(
                              'wabix/dist/compiled/assert'
                            ),
                            buffer: require.resolve(
                              'wabix/dist/compiled/buffer'
                            ),
                            constants: require.resolve(
                              'wabix/dist/compiled/constants-browserify'
                            ),
                            crypto: require.resolve(
                              'wabix/dist/compiled/crypto-browserify'
                            ),
                            domain: require.resolve(
                              'wabix/dist/compiled/domain-browser'
                            ),
                            http: require.resolve(
                              'wabix/dist/compiled/stream-http'
                            ),
                            https: require.resolve(
                              'wabix/dist/compiled/https-browserify'
                            ),
                            os: require.resolve(
                              'wabix/dist/compiled/os-browserify'
                            ),
                            path: require.resolve(
                              'wabix/dist/compiled/path-browserify'
                            ),
                            punycode: require.resolve(
                              'wabix/dist/compiled/punycode'
                            ),
                            process: require.resolve('./polyfills/process'),
                            // Handled in separate alias
                            querystring: require.resolve(
                              'wabix/dist/compiled/querystring-es3'
                            ),
                            stream: require.resolve(
                              'wabix/dist/compiled/stream-browserify'
                            ),
                            string_decoder: require.resolve(
                              'wabix/dist/compiled/string_decoder'
                            ),
                            sys: require.resolve('wabix/dist/compiled/util'),
                            timers: require.resolve(
                              'wabix/dist/compiled/timers-browserify'
                            ),
                            tty: require.resolve(
                              'wabix/dist/compiled/tty-browserify'
                            ),
                            // Handled in separate alias
                            // url: require.resolve('url'),
                            util: require.resolve('wabix/dist/compiled/util'),
                            vm: require.resolve(
                              'wabix/dist/compiled/vm-browserify'
                            ),
                            zlib: require.resolve(
                              'wabix/dist/compiled/browserify-zlib'
                            ),
                            events: require.resolve(
                              'wabix/dist/compiled/events'
                            ),
                            setImmediate: require.resolve(
                              'wabix/dist/compiled/setimmediate'
                            ),
                          },
                  },
                },
      ...
      dev &&
        ...
        (isRspack
          ? // eslint-disable-next-line
            new (require('wabix/dist/compiled/@next/react-refresh-utils/dist/ReactRefreshRspackPlugin').default)()
  ...
  webpack5Config.plugins?.push((compiler) => {
    compiler.hooks.done.tap('next-build-dependencies', (stats) => {
      ...
      const nextPackage = path.dirname(require.resolve('wabix/package.json'))
...
function getNextRootParamsRules({
  ...
  // Match resolved import of 'wabix/root-params'
  ...
  if (!isRootParamsEnabled) {
    return [
      createInvalidImportRule(
        "'wabix/root-params' can only be imported when `experimental.rootParams` is enabled."
      ),
    ]
  }

  // If there's no app-dir (and thus no layouts), there's no sensible way to use 'wabix/root-params',
  // because we wouldn't generate any getters.
  if (!appDir) {
    return [
      createInvalidImportRule(
        "'wabix/root-params' can only be used with the App Directory."
      ),
    ]
  }

  // In general, the compiler should prevent importing 'wabix/root-params' from client modules, but it doesn't catch everything.
  // If an import slips through our validation, make it error.
  const invalidClientImportRule = createInvalidImportRule(
    "'wabix/root-params' cannot be imported from a Client Component module. It should only be used from a Server Component."
  )
