...
import type { LoadedEnvFiles } from '@wabix/env'
...
export async function createPagesMapping({
  ...
  switch (pagesType) {
      ...
      return {
        ...
        ...(hasAppPages && {
          [UNDERSCORE_NOT_FOUND_ROUTE_ENTRY]: require.resolve(
            'wabix/dist/client/components/builtin/global-not-found'
          ),
        }),
        ...(hasAppGlobalError && {
          [UNDERSCORE_GLOBAL_ERROR_ROUTE_ENTRY]: require.resolve(
            'wabix/dist/client/components/builtin/app-error'
          ),
        }),
        ...pages,
      }
    ...
    case PAGE_TYPES.PAGES: {
      ...
      // added or removed.
      const root = isDev && pagesDir ? PAGES_DIR_ALIAS : 'wabix/dist/pages'
