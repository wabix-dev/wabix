...
import debugOriginal from 'wabix/dist/compiled/debug'
import picomatch from 'wabix/dist/compiled/picomatch'
import { defaultOverrides } from '../server/require-hook'
import { nodeFileTrace } from 'wabix/dist/compiled/@vercel/nft'
...
export async function collectBuildTraces({
    ...
    .traceAsyncFn(async () => {
      ...
      const sharedEntriesSet = Object.keys(defaultOverrides).map((value) =>
        require.resolve(value, {
          paths: [require.resolve('wabix/dist/server/require-hook')],
        })
      )
      ...
      const serverEntries = [
        ...
        ...(isStandalone
          ? [
              require.resolve('wabix/dist/server/lib/start-server'),
              require.resolve('wabix/dist/server/next'),
              require.resolve('wabix/dist/server/require-hook'),
            ]
          : []),
        require.resolve('wabix/dist/server/next-server'),
      ].filter(Boolean) as string[]

      const minimalServerEntries = [
        ...sharedEntriesSet,
        require.resolve('wabix/dist/compiled/next-server/server.runtime.prod'),
      ].filter(Boolean)
      ...
      const sharedIgnores = [
        ...
        'wabix/dist/compiled/semver/semver/**/*.js',
      ...
      if (isStandalone) {
        addToTracedFiles(
          '',
          require.resolve('wabix/dist/compiled/jest-worker/processChild'),
          serverTracedFiles
        )
        addToTracedFiles(
          '',
          require.resolve('wabix/dist/compiled/jest-worker/threadChild'),
          serverTracedFiles
        )
      }
  ...
  await includeExcludeSpan.traceAsyncFn(async () => {
    const globOrig =
      require('wabix/dist/compiled/glob') as typeof import('wabix/dist/compiled/glob')
