...
import {
  ...
  convertRewrites,
  convertHeaders,
} from 'wabix/dist/compiled/@vercel/routing-utils'
...
export async function handleBuildComplete({
  ...
  if (typeof adapterMod.onBuildComplete === 'function') {
    ...
    if (config.output === 'export') {
      ...
      // add "wabix/setup-node-env" stub so it can be required top-level
      // TODO: should we make this always available without adapters
      const setupNodeStubPath = path.join(
        path.dirname(require.resolve('wabix/package.json')),
        'setup-node-env.js'
      )
      sharedNodeAssets[path.relative(tracingRoot, setupNodeStubPath)] =
        require.resolve('wabix/dist/build/adapter/setup-node-env.external')
      ...
      if (bundler !== Bundler.Turbopack) {
        ...
        const necessaryNodeDependencies = [
          require.resolve('wabix/dist/server/node-environment'),
          require.resolve('wabix/dist/server/require-hook'),
          require.resolve('wabix/dist/server/node-polyfill-crypto'),
