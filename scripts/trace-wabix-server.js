...
async function main() {
  ...
  const nextServerPath = path.join(
    workDir,
    'node_modules/next/dist/server/wabix-server.js'
  )
  ...
  await fsp.writeFile(
    path.join(
      __dirname,
      '../packages/wabix/dist/server/wabix-server.js.nft.json'
    ),
    JSON.stringify({
  ...
  if (
    ...
    throw new Error(
      `Max traced size of wabix-server exceeded limits of ${MAX_COMPRESSED_SIZE} compressed or ${MAX_UNCOMPRESSED_SIZE} uncompressed`
    )
  }
}
