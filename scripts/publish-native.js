...
;(async function () {
  try {
    ...
    await Promise.all(
      platforms.map(async (platform) => {
        ...
        try {
          ...
          await cp(
            path.join(cwd, 'packages/wabix-swc/native', binaryName),
    ...
    let nextPkg = JSON.parse(
      await readFile(path.join(cwd, 'packages/wabix/package.json'))
    )
    ...
    await writeFile(
      path.join(path.join(cwd, 'packages/wabix/package.json')),
